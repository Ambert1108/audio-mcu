...
namespace aesir {
    ...
    private:
        ...
        const int chunkSize = 32000;          // 处理块大小（样本数，默认3000≈188ms）
    public:
        TranscriberUnit() {
        ...
        bool init(const std::string& modelFile = "/home/blueFlower/workspace/aesir/whispercpp/whisper.cpp-master/models/ggml-large-v3-turbo.bin") {
            ...
            wparams.no_speech_thold = 0.0; // 从默认0.6降至0.3-0.4
            ...
            wparams.initial_prompt = "以下是普通话的句子，请使用简体中文和规范标点符号。"; // 提升简体中文识别准确率
            //wparams.initial_prompt = "请忽略杂音"; 
            //wparams.beam_search.beam_size = 5;     // 默认2，实时场景不超过10
            //wparams.temperature = 0.0; // 关闭随机性，避免错误生成
            wparams.max_len = 20;  // 每行最多20字符（中文）
            ...
            //VAD
            wparams.vad = true;
            wparams.vad_model_path = "/home/blueFlower/workspace/aesir/whispercpp/whisper.cpp-master/models/ggml-silero-v5.1.2.bin";  // 模型路径
            whisper_vad_params vadPara = {
                .threshold = 0.7,                  // 宽松阈值，避免漏掉轻声
                .min_speech_duration_ms = 800,      // 过滤短暂杂音
                .min_silence_duration_ms = 800,     // 长静音分段保证语义连贯
                .max_speech_duration_s = 20.0,      // 限制单段长度
                .speech_pad_ms = 300,               // 中等填充防截词
                .samples_overlap = 0.05             // 小幅重叠减少拼接痕迹
            };
            wparams.vad_params = vadPara;
